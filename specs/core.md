...
The core routing engine classifies incoming LLM prompts by semantic similarity to configured capabilities, routes them to the most appropriate backend, and logs every decision to SQLite for analytics.
...
| `server.port` | integer | no | `8321` | Listening port for the proxy |
...
| `dashboard.enabled` | boolean | no | `true` | Whether to start the dashboard server |
| `dashboard.db_path` | string | no | `"switchyard.db"` | Path to SQLite database file |
| `dashboard.port` | integer | no | `8421` | Dashboard HTTP port |
...
8. Log the routing decision (see §3).
...
## 5. HTTP Proxy Interface

The core engine exposes an OpenAI-compatible proxy endpoint:
...
**Response:** Proxied response from the selected backend, with an additional `X-Switchyard-Category` header indicating the routing decision.
